<template>
  <!-- 首页 -->
  <div>
    <!-- 头部 -->
    <div class="header">
      <div class="logo"></div>
      <div class="search" @click="$router.push('/search')">
        <i class="iconfont icon-icon-test"></i>
        <span class="placeholder">搜索商品,共{{total}}款好物</span>
      </div>
      <div v-if="userinfo" @click="goCart">
        <i class="iconfont icon-gouwuchekong"></i>
        <span class="cart-num" v-if="cartNum > 0">{{cartNum}}</span>
      </div>
      <div v-else class="login" @click="login">登录</div>
    </div>
    <!-- 触底加载滚动的区域 -->
    <div class="scroll-container" ref="scroll-container">
      <div class="scroll-inner" ref="scroll-inner">
        <!-- 轮播图 -->
        <!-- 传值传到Swiper中 -->
        <Swiper height="3.7rem" :list="bannerList" class="swiper"/>

        <!-- 首页信息 -->
        <!-- 服务信息 -->
        <!-- 当info && info.commonConfigModule && info.commonConfigModule.brandDescPicUrl都存在时才取值加载 -->
        <div class="service-policy" v-if="info && info.commonConfigModule && info.commonConfigModule.brandDescPicUrl">
          <img :src="info.commonConfigModule.brandDescPicUrl" alt="">
        </div>
        <!-- 分类 -->
        <ul class="kingkong" v-if="info && info.kingKongAreaV4">
          <!-- slice(0,10):截取前十个 -->
          <li class="kingkong-item" v-for="(item,index) in info.kingKongAreaV4.slice(0,10)" :key="index">
            <img :src="item.picUrls[0]" alt="">
            <div class="title">{{item.title}}</div>
          </li>
        </ul>
        <!-- banner -->
        <!-- <div class="operation-cfg" v-if="info && info.operationCfg && info.operationCfg[0]">
          <img :src="info.operationCfg[0].picUrls[0]" alt />
        </div> -->
        <div class="operation-cfg">
          <img src="@/assets/img/profile-bg.png" alt="">
        </div>

        <!-- 推荐商品 -->
        <div class="rcmd" v-if="rcmdItemList.length>0">
          <div class="rcmd-title">推荐商品</div>
          <div class="rcmd-list">
            <product v-for="(item, index) in rcmdItemList" :key="index" :item="item.categoryItem" />
          </div>
        </div>
      </div>
    </div>

    <!-- 底部导航 -->
    <nav-footer />

    <!-- 加载中页面效果 -->
    <loading v-show="isLoading"/>

    <!-- 回到顶部 -->
    <div class="to-top" @click="backToTop"></div>
  </div>
</template>
<script>
import NavFooter from '../components/NavFooter'
import Swiper from '../components/Swiper'
import Product from '../components/Product'
import Loading from '../components/Loading'
import { API_HOME, API_HOME_TOTAL_NUM, API_HOME_RCMD, API_CART_NUM } from '@/api.config.js'
import { mapState, mapMutations } from 'vuex'

export default {
  data () {
    return {
      total: 0, // 当前商品总数
      bannerList: [
        // 'https://yanxuan.nosdn.127.net/e48cc1f908b359c891bb0152feb9e50e.jpg?type=webp&imageView&quality=75&thumbnail=750x0',
        // 'https://yanxuan.nosdn.127.net/9f40ca5a83a06ded9587124432471b67.jpg?type=webp&imageView&quality=75&thumbnail=750x0',
        // 'https://yanxuan.nosdn.127.net/7f762f223c3e320df8260c4b54879124.jpg?type=webp&imageView&quality=75&thumbnail=750x0',
        // 'https://yanxuan.nosdn.127.net/ef739c5f6ba97afbf201ee9d9b3eb16d.jpg?type=webp&imageView&quality=75&thumbnail=750x0',
        // 'https://yanxuan.nosdn.127.net/7d38ac617e6175f468140d326b7ebcda.jpg?type=webp&imageView&quality=75&thumbnail=750x0',
        // 'https://yanxuan.nosdn.127.net/e52ad10cfd0f24691c987006ef82a814.jpg?type=webp&imageView&quality=75&thumbnail=750x0'
      ],
      info: {}, // 首页信息
      rcmdItemList: [], // 推荐商品列表
      isLoading: false
    }
  },
  computed: {
    ...mapState(['cartNum', 'userinfo']) // 解构属性
  },
  components: { NavFooter, Swiper, Product, Loading },
  methods: {
    ...mapMutations(['initCartNum', 'setUserinfo']), // 解构方法
    swiperClick (item) {
      console.log(item)
    },
    async getTotalNums () {
      // 方法一
      // const res = await this.$axios.get('http://yanxuan.xiecheng.live:7001/getTotalNums')// 异步操作
      // console.log(res)
      // this.total = res.data.data
      // 方法二
      // const {
      //   data: { data }
      // } = await this.$axios.get('http://yanxuan.xiecheng.live:7001/getTotalNums')
      // this.total = data
      // 方法三
      this.isLoading = true
      const total = await this.$axios.get(API_HOME_TOTAL_NUM)
      this.total = total
      this.isLoading = false
    },
    async getHomeinfo () {
      const res = await this.$axios.get(API_HOME)
      // console.log(res)
      this.info = res
      this.bannerList = res.focus.map(item => item.img) // 轮播图
    },
    async getRcmdInfo (lastItemId = 0, size = 20) {
      const { rcmdItemList } = await this.$axios.post(API_HOME_RCMD, {
        lastItemId, // 开始条数
        size // 一次取多少条
      })
      // console.log(res)
      // this.rcmdItemList = this.rcmdItemList
      this.rcmdItemList = this.rcmdItemList.concat(rcmdItemList)// 将原有数据上与后加载的数据连接起来
    },
    initScroll () {
      const scrollTop = this.scrollContainer.scrollTop // 滚动出去部分的高度
      this.scrollInnerH = this.scrollInner.scrollHeight // 页面高度
      // console.log(scrollTop + this.scrollContainerH, this.scrollInnerH)
      if (scrollTop + this.scrollContainerH + 1 >= this.scrollInnerH) {
        // console.log('触底')
        // 取到最后一条数据
        this.getRcmdInfo(this.rcmdItemList[this.rcmdItemList.length - 1].id)
      }
    },
    // 防抖函数,传入参数，哪一个函数防抖，延迟时间
    debounce (fn, delay) {
      let timer = null
      return () => {
        if (timer !== null) {
          clearTimeout(timer)
        }
        timer = setTimeout(fn, delay)
      }
    },
    // 节流，传入参数，哪一个函数节流，延迟时间
    // 相对于防抖方式来说停止后间隔设定时间输出进行，节流是隔一段指定时间计算输出
    throttle (fn, delay) {
      let startTime = 0
      return () => {
        const nowTime = Date.new() // 当前时间
        if (nowTime - startTime >= delay) { // 当前时间-开始时间是否到达给定时间
          startTime = nowTime
          fn()
        }
      }
    },
    // 回到顶部
    backToTop () {
      const timer = setInterval(() => {
        this.scrollContainer.scrollTop = this.scrollContainer.scrollTop - 100
        if (this.scrollContainer.scrollTop === 0) {
          clearInterval(timer)
        }
      }, 17)
    },
    async getCartNum () { // 获取当前购物车商品数量
      const res = await this.$axios.get(API_CART_NUM)
      if (res) {
        // res.num:为当前购物车中的商品数量
        // res.user:为用户信息
        this.initCartNum(res.num)
        this.setUserinfo(res.user)
      }
    },
    login () {
      this.$router.push('/profile/login')
    },
    goCart () {
      this.$router.push('/cart')
    }
  },
  created () {
    this.getTotalNums()
    this.getHomeinfo()
    this.getRcmdInfo()
    this.getCartNum()
  },
  mounted () {
    this.$nextTick(() => {
      this.scrollContainer = this.$refs['scroll-container']
      this.scrollInner = this.$refs['scroll-inner']
      this.scrollContainerH = this.scrollContainer.clientHeight
      this.scrollContainer.addEventListener('scroll', this.debounce(this.initScroll, 500))
    })
  },
  destroyed () {
    this.scrollContainer.removeEventListener('scroll', this.initScroll)
  }
}
</script>

<style lang="scss" scoped>
.header{
  height:0.88rem;
  padding: 0 0.3rem;
  display: flex;
  align-items: center;
  position: fixed;
  z-index: 99;
  left: 0;
  top: 0;
  width: 100%;
  background-color: $colorA;
  box-sizing: border-box;
  .logo{
    width: 1.38rem;
    height: 0.4rem;
    background-image: url('../assets/img/logo.png');
    background-size: cover;
  }
  .search{
    flex: 1;
    background-color: #ededed;
    margin: 0 0.2rem;
    display: flex;
    justify-content: center;
    align-content: center;
    .icon-icon-test{
      font-size: $fontB;
      color: $colorC;
      margin-left: 0.1rem;
    }
    .placeholder{
      font-size: $fontB;
      color: $colorF;
    }
  }
  .login{
    width: 0.72rem;
    height: 0.38rem;
    line-height: 0.38rem;
    text-align: center;
    color: $colorB;
    border: 1px solid $colorB;
    border-radius: 0.08rem;
    font-size: $fontB;
  }
}
// .swiper{
//   margin-top: 0.88rem;
// }
.kingkong{
  display: flex;
  justify-content: space-around;
  flex-wrap: wrap;//一行放不下自动换行
  &-item{
    width: 1.1rem;
    height: 1.56rem;
    margin: 0.1rem 0.2rem;
    text-align: center;
    img{
      width: 100%;
    }
    .title{
      color: $colorC;
      font-size: $fontA;
      white-space: nowrap;//不让文字换行
    }
  }
}
.operation-cfg{
  width: 100%;
  height: 2.3rem;
  img{
    width: 100%;
    height: 100%;
  }
}
.rcmd{
  background-color: $colorA;
  margin-bottom: 0.97rem;
  &-title{
    text-align: center;
    padding: 0.2rem 0;
    font-weight: 700;
  }
  &-list{
    display: flex;
    flex-wrap: wrap;
    justify-content: space-between;
    padding: 0 0.2rem;
  }
}
.scroll{
  &-container{
    position: absolute;
    top: 0.88rem;
    bottom: 0.97rem;
    left: 0;
    right: 0;
    overflow-y: scroll;//垂直方向上就会产生一个滚动条
  }
}
.to-top{
  width: 0.8rem;
  height: 0.8rem;
  background-image: url('../assets/img/to-top.png');
  background-size: 0.8rem 0.8rem;
  position: fixed;
  right: 0.2rem;
  bottom: 1.4rem;
  z-index: 99;
}
.cart{
  position: relative;
  .iconfont{
    font-size: 0.86rem;
  }
  &-num{
    width: 0.36rem;
    height: 0.36rem;
    background-color: $colorB;
    text-align: center;
    line-height: 0.36rem;
    border-radius: 50%;
    color: $colorA;
    position: absolute;
    top: 0.1rem;
    right: 0.01rem;
    font-size: $fontB;
  }
}
</style>
